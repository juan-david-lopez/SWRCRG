          ...
          {notifs.length === 0 ? (
            ...
            <ul style={styles.list}>
              {notifs.map((n) => (
                <li
                  key={n.id}
                  style={{ ...styles.item, background: n.leida ? 'var(--c-surface)' : 'var(--c-primary-bg)' }}
...
const styles = {
  ...
  dropdown:   { position: 'absolute', right: 0, top: '36px', width: '300px', background: 'var(--c-surface)', borderRadius: '12px', boxShadow: '0 4px 24px rgba(0,0,0,0.12)', zIndex: 100, overflow: 'hidden', border: '1px solid var(--c-border)' },
  dropHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderBottom: '1px solid var(--c-border)' },
  dropTitle:  { fontWeight: '700', fontSize: '14px', color: 'var(--c-text)' },
  ...
  item:       { padding: '12px 16px', borderBottom: '1px solid var(--c-bg)', cursor: 'pointer', position: 'relative' },
  itemTitle:  { margin: '0 0 2px', fontSize: '13px', fontWeight: '600', color: 'var(--c-text)' },
  itemMsg:    { margin: 0, fontSize: '12px', color: 'var(--c-text-2)' },
  dot:        { position: 'absolute', top: '14px', right: '12px', width: '8px', height: '8px', borderRadius: '50%', background: '#2563eb' },
  empty:      { padding: '20px', textAlign: 'center', color: 'var(--c-text-3)', fontSize: '13px' },
};
